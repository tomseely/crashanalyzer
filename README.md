...
- Installation and setup instructions
    ...
    - However if it is private then the website can still be locally hosted on the machine with a Gemini API Key and cloning the repository
        - This can be done by changing the environment variable by doing the following command in the terminal: `GENAI_API_KEY=your_key_here` or by adding it to .apikey.txt
- How to run the program and reproduce results
    ...
    - Run `python3 ./tests/app.py` and open the local port in the browser to view the website.
